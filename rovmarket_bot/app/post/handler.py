...
async def process_price(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📞 Пожалуйста, отправьте ваши контактные данные или свяжитесь с нами анонимно через бот:\n\n"
        "— Номер телефона (начиная с `+7`, `+380` или `+8`)\n"
        "— Email (например, `example@mail.com`)\n"
        "— Telegram username (начиная с `@`, например `@username`)\n\n"
        "Если вы хотите остаться анонимным, используйте кнопку «💬 Связаться через бота» ниже — ваш контакт не будет виден 👇",
...
async def price_negotiable_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "— Или свяжитесь с нами анонимно через бот — ваш контакт не будет виден 👤\n\n"
