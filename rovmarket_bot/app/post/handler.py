...
async def process_price(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📞 Пожалуйста, отправьте ваши контактные данные или позвольте покупателям написать вам через бота анонимно:\n\n"
        "— Номер телефона (начиная с `+7`, `+380` или `+8`)\n"
        "— Email (например, `example@mail.com`)\n"
        "— Telegram username (начиная с `@`, например `@username`)\n\n"
        "Если вы хотите остаться анонимным для покупателей, используйте кнопку «💬 Связаться через бота» ниже — они не увидят ваши контакты 👇",
...
async def price_negotiable_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "Пожалуйста, укажите, как с вами можно связаться:\n"
        "— Телефон (начиная с `+7`, `+380` или `+8`)\n"
        "— Email (например, `example@mail.com`)\n"
        "— Telegram (начиная с `@`, например `@username`)\n"
        "— Или дайте возможность покупателям написать вам анонимно через бот — они не увидят ваши контакты 👤\n\n"
        "Для быстрой отправки номера используйте кнопку «📱 Отправить номер телефона» ниже 👇",
        reply_markup=contact,
    )
